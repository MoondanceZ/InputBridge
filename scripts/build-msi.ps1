...
$project = Join-Path $root "InputBridge.csproj"
...
function Get-ProjectVersion {
    ...
    if ([string]::IsNullOrWhiteSpace($version)) {
        throw "InputBridge.csproj does not contain a <Version> value."
    }
...
if (-not $SkipPublish) {
    Write-Host "Publishing InputBridge AOT ($Configuration, $Runtime)..."
...
$msi = Join-Path $artifacts "InputBridge-$version-x64.msi"
